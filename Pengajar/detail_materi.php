...
if (!$id_pengguna || !$role || ((!$materi_id || !$kelas_id) && !$modul_id)) {
    echo "<p class='text-red-500 text-center font-bold mt-10'>Data tidak lengkap atau tidak valid.</p>";
    exit;
}
...
if ($modul_id) {
    // Modul hanya untuk murid yang sudah login
    $aksesDiizinkan = $role === 'murid';
} else {
    if ($role === 'pengajar') {
...
if (!$aksesDiizinkan) {
    echo "<p class='text-red-500 text-center font-bold mt-10'>Anda tidak memiliki akses ke konten ini.</p>";
    exit;
}
...
if ($modul_id) {
    ...
    // Ambil nama kelas juga
    $stmt_kelas = $conn->prepare("SELECT judul FROM kelas WHERE kelas_id = ?");
    $stmt_kelas->bind_param("i", $kelas_id);
    $stmt_kelas->execute();
    $stmt_kelas->bind_result($judulKelas);
    $stmt_kelas->fetch();
    $stmt_kelas->close();
}
...
if (!$stmt->fetch()) {
    echo "<p class='text-red-500 text-center font-bold mt-10'>Data tidak ditemukan.</p>";
    exit;
}
$stmt->close();
?>
            ...
            <div class="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
                <div class="bg-teal-500 text-white p-6 rounded-t-lg shadow-md">
                    <h1 class="text-2xl md:text-3xl font-medium">
                        <?= $modul_id ? "Modul Pembelajaran" : htmlspecialchars($judulKelas) ?>
                    </h1>
